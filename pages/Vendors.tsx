...
const Vendors = () => {
  ...
  const loadVendors = async () => {
    setLoading(true);
    const data = await db.fetch(db.getPartners('vendor'));
    setVendors(data);
    setLoading(false);
  };
  ...
  const filteredData = vendors.filter(v => v.name.toLowerCase().includes(search.toLowerCase()) || v.email.toLowerCase().includes(search.toLowerCase()) || v.phone.includes(search));
  ...
  const openModal = (vendor?: Partner) => {
      setEditingVendor(vendor || null);
      setFormData(vendor ? { ...vendor } : { name: '', email: '', phone: '', address: '', ntn: '', strn: '', cnic: '' });
      setErrors({});
      setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
      ...
      try {
          db.savePartner({ ...formData, type: 'vendor', accountId: '2111' });
  ...
  return (
    <div className="space-y-6">
       <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div><h1 className="text-2xl font-bold text-text-main tracking-tight">Vendors</h1><p className="text-text-muted text-sm mt-1">Manage suppliers and accounts payable.</p></div>
          <button onClick={() => openModal()} className="flex items-center gap-2 bg-primary hover:bg-primary-light text-white px-5 py-2.5 rounded-xl font-medium shadow-lg shadow-primary/25 transition-all active:scale-95"><Plus size={18} /> Add Vendor</button>
       </div>
       <div className="bg-surface rounded-2xl shadow-soft border border-border overflow-hidden">
          <div className="p-5 border-b border-border bg-surface/50 flex flex-col sm:flex-row gap-4 justify-between items-center">
              <div className="relative w-full sm:w-80 group"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-primary transition-colors" size={16} /><input type="text" placeholder="Search vendors..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-surface-highlight border border-transparent rounded-xl py-2.5 pl-10 pr-4 text-sm text-text-main focus:border-primary/30 focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all"/></div>
          </div>
          <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                  <thead className="bg-surface-highlight border-b border-border">
                      <tr>
                          {[
                              { key: 'name', label: 'Vendor Name', w: '30%' },
                  ...
                  <tbody className="divide-y divide-border">
                      {loading ? (<tr><td colSpan={5} className="p-8 text-center text-text-muted animate-pulse">Loading vendors...</td></tr>) : paginatedData.length === 0 ? (<tr><td colSpan={5} className="p-12 text-center text-text-muted">No vendors found.</td></tr>) : (
                          paginatedData.map((vendor) => (
                              <tr key={vendor.id} className="hover:bg-surface-highlight transition-colors group">
                                  <td className="px-6 py-4"><div className="font-semibold text-text-main">{vendor.name}</div><div className="text-xs text-text-muted mt-0.5 font-mono opacity-70">ID: {vendor.id}</div></td>
                                  <td className="px-6 py-4"><div className="flex flex-col gap-1.5">{vendor.email && <div className="flex items-center gap-2 text-text-muted text-xs"><Mail size={12}/> {vendor.email}</div>}{vendor.phone && <div className="flex items-center gap-2 text-text-muted text-xs"><Phone size={12}/> {vendor.phone}</div>}</div></td>
                                  <td className="px-6 py-4 text-text-muted truncate max-w-xs" title={vendor.address}>{vendor.address}</td>
                                  <td className="px-6 py-4 text-right"><span className={`font-mono font-bold ${vendor.balance > 0 ? 'text-danger' : 'text-text-main'}`}>${vendor.balance.toLocaleString()}</span></td>
                                  <td className="px-6 py-4 text-right">
                                      <div className="flex items-center justify-end gap-2">
                                          <button onClick={() => openModal(vendor)} className="p-2 text-blue-500 hover:bg-blue-500/10 rounded-lg transition-colors"><Edit2 size={16} /></button>
                                          <button onClick={(e) => handleDelete(e, vendor.id)} className="p-2 text-danger hover:bg-danger/10 rounded-lg transition-colors"><Trash2 size={16} /></button>
                                      </div>
                                  </td>
                              </tr>
                          ))
       ...
       <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingVendor ? 'Edit Vendor' : 'Add New Vendor'} maxWidth="max-w-2xl">
          ...
          <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-border"><button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-medium text-text-muted hover:text-text-main transition-colors">Cancel</button><button onClick={handleSubmit} disabled={isSaving} className="flex items-center gap-2 px-6 py-2 rounded-xl bg-primary text-white text-sm font-medium hover:bg-primary-light shadow-lg shadow-primary/20 disabled:opacity-70 transition-all active:scale-95">{isSaving ? 'Saving...' : <><Save size={16} /> Save Vendor</>}</button></div>
       </Modal>
    </div>
  );
};
